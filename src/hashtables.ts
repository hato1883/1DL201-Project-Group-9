 ...
 * difficult to invert or predict.
 ...
 * @returns the associated value, or undefined if it does not exist.
 */
export function ch_lookup<K, V>(
    { arr, hash }: ChainingHashtable<K, V>,
    key: K
): V | undefined {
...
export function ch_insert<K, V>(
    ...
    if (scan(arr[index], key) === undefined) {
        ...
        arr[index] = map(
            (kv) => (key === head(kv) ? pair(key, value) : kv),
            arr[index]
        );
        return true;
    }
...
// double hashing
export function probe_double<K>(
    base_hash: HashFunction<K>,
    probe_hash: HashFunction<K>
): ProbingFunction<K> {
    function probe(length: number, key: K, i: number): number {
...
function probe_from<K, V>(
    { keys, probe }: ProbingHashtable<K, V>,
    key: K,
    i: number
): number | undefined {
 ...
 * @returns the associated value, or undefined if it does not exist.
 */
export function ph_lookup<K, V>(
    tab: ProbingHashtable<K, V>,
    key: K
): V | undefined {
...
export function ph_insert<K, V>(
    ...
    function insert_from(i: number): boolean {
        ...
        if (tab.keys[index] === key || tab.keys[index] === undefined) {
            ...
            return insert_at(location === undefined ? index : location);
        } else {
 ...
 */
function filter_nulls<T>(xs: List<T | undefined | null>): List<T> {
    if (is_null(xs)) {
        return null;
    } else {
        const x = head(xs);
        if (x === undefined || x === null) {
            return filter_nulls(tail(xs));
        } else {
            return pair(x, filter_nulls(tail(xs)));
        }
    }
}
...
export function ph_keys<K, V>(tab: ProbingHashtable<K, V>): List<K> {
    return filter_nulls(build_list((i) => tab.keys[i], tab.keys.length));
}
